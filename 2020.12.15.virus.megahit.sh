...
#assembly
#ls *R1.fq | while read id;do(mkdir -p genome_assembly/assembly_result/$(basename $id ".unmapped.R1.fq");spades.py -k 21,33,55,77 --only-assembler --pe1-1 $id --pe1-2 $(basename $id "R1.fq")R2.fq -o genome_assembly/assembly_result/$(basename $id ".unmapped.R1.fq"));done
#filter scaffolds (gt 500)
#ls *R1.fq.gz | while read id;do(python /media/ruizhi/software/min_500.py -i genome_assembly/spades/$(basename $id "_R1.fq.gz")/scaffolds.fasta -o genome_assembly/spades/$(basename $id "_R1.fq.gz")/scaffolds.500.fasta);done
...
ls *R1.fq | while read id;do(awk -F ' ' 'NR%2==1{print $1"\t"$NF}' genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$id > genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast);done
ls *R1.fq | while read id;do(sed -i 's/>//g' genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast);done
ls *R1.fq | while read id;do(sed -i 's/len=//g' genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast);done
ls *R1.fq | while read id;do(sort -nk 2 genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast > genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast.xl);done
echo -e "contig\tlen" > head
cat head genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast.xl > genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/$(basename $id 'fa')stast.xls
rm head genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/*stast genome_assembly/2.Assembly/$(basename $id '.unmapped.R1.fq')/*stast.xl
